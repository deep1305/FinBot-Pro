...
class DataIngestion:
    ...
    def store_in_vector_db(self, documents: List[Document]):
        try:
            ...
            # If index exists, check dimensions and delete if they don't match
            if index_name in existing_indexes:
                try:
                    index = pinecone_client.Index(index_name)
                    index_stats = index.describe_index_stats()
                    current_dimension = index_stats.get("dimension")
                    
                    if current_dimension and current_dimension != embedding_dimension:
                        print(f"Existing index has dimension {current_dimension}, but we need {embedding_dimension}. Deleting index...")
                        pinecone_client.delete_index(index_name)
                        print(f"Index {index_name} deleted successfully.")
                except Exception as e:
                    print(f"Error checking index dimensions: {e}")
                    # If we can't check dimensions, delete to be safe
                    pinecone_client.delete_index(index_name)
                    print(f"Index {index_name} deleted due to error.")
            ...
            if index_name not in [i.name for i in pinecone_client.list_indexes()]:
                ...
                pinecone_client.create_index(
                    name=index_name,
                    dimension=embedding_dimension,  # text-embedding-3-large dimensions
                    metric="cosine",
